...
describe Ray do
  ...
  describe "#relative_position/#point_to_the_left?" do
    let(:ray) { Ray.new(Vector2D[0, 0], Vector2D[0, 3]) }
  ...
  describe "#angle_to" do
    ...
    it "calculates the angle to a ray" do
      other_ray = Ray.new(Vector2D[0, 0], Vector2D[0, -1])
      ray.angle_to(other_ray).must_be_close_to Math::PI, 1e-6
    end
  end

  describe "#right_handed?" do
    let(:ray) { Ray.new(Vector2D[0, 0], Vector2D[1, 0]) }
  ...
  describe "#==" do
    it "returns true when both the point and displacement vectors are identical" do
      Ray.new(Vector2D[0, 0], Vector2D[0, 1]).must_equal Ray.new(Vector2D[0, 0], Vector2D[0, 1])

      Ray.new(Vector2D[0, 0], Vector2D[0, 1]).wont_equal Ray.new(Vector2D[2, 0], Vector2D[0, 1])
      Ray.new(Vector2D[0, 0], Vector2D[0, 1]).wont_equal Ray.new(Vector2D[0, 0], Vector2D[1, 1])
    end

    it "handles equivalent rays with differing representations" do
      Ray.new(Vector2D[0, 0], Vector2D[0, 1]).must_equal Ray.new(Vector2D[0, 0], Vector2D[0, 2])
    end
  end

  describe "#rotate" do
    it "rotates the ray" do
      ray = Ray.new(Vector2D[0, 0], Vector2D[0, 1])
      ...
      rotated_ray.point.must_equal Vector2D[0, 0]
    end
  end

  describe "#intersection" do
    it "returns the point of intersection with another ray" do
      r1 = Ray.new(Vector2D[1, 0], Vector2D[1, 0])
      r2 = Ray.new(Vector2D[1, 1], Vector2D[0, 1])

      r1.intersection(r2).must_equal Vector2D[1, 0]
    end

    it "returns nil for parallel rays" do
      r1 = Ray.new(Vector2D[1, 0], Vector2D[1, 0])
      r2 = Ray.new(Vector2D[2, 0], Vector2D[1, 0])
